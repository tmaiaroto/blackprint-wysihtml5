...
NOTE: This fork is not supported by XING AG. It is meant for use with the Blackprint CMS and is maintained by Shift8Creative.
The original README has been left in place for now (it will eventually be updated and replaced).
...
Ensure you install that (npm install -g minify) and then you should be ok. The original build required Node.js anyway. Now you just need to
ensure you have that package. The Makefile has changed a little bit, but building is the same, just run: make
